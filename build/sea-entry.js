...
// VBS instead, and wscript starts the proxy with no window at all, which is the
// same trick scripts/gui.vbs uses for the GUI.
...
// CRLF on purpose: this goes to the VBScript parser, and the file is easier to read
// in Notepad when it looks like every other .vbs on the machine.
function autostartScriptText(exePath) {
  const launch = `${vbsQuote(exePath)} --proxy`;
  return [
...
function main() {
  ...
  // bare `ReasoningProxy.exe --proxy` takes, which is what the sign-in launcher runs.
  // chooseDataDir keeps that working when the exe sits in a read-only folder.
