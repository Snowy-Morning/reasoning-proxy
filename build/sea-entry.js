...
// A one-file portable exe has nothing to double click, and "run a flag from a
// terminal" is not something a person discovers by looking at a folder. The first
// portable start therefore drops uninstall.bat next to the exe; --purge removes it
// with the rest. The first line below is the marker that says the file is ours, so a
// hand-written script with the same name is never overwritten or deleted.
const UNINSTALL_BAT_NAME = "uninstall.bat";
const UNINSTALL_BAT_MARKER = /^rem ReasoningProxy uninstaller/mi;

function uninstallBatText(exeName) {
  return (
    [
      "@echo off",
      `rem ReasoningProxy uninstaller, generated by ${exeName}`,
      "rem If this file is gone, uninstall from Settings > Apps, or run:",
      `rem   ${exeName} --uninstall --purge`,
      "setlocal",
      `set "EXE=%~dp0${exeName}"`,
      "",
      "if not exist \"%EXE%\" (",
      "  echo Not found: %EXE%",
      "  echo The program is already gone, so this script has no job left.",
      "  pause",
      "  exit /b 1",
      ")",
      "",
      "echo Removing Reasoning Proxy:",
      "echo   its settings      config",
      "echo   its logs          logs",
      "echo   model config backups",
      "echo   the Settings > Apps entry",
      "echo   the program itself",
      "echo.",
      "echo VS Code keeps its own chatLanguageModels.json untouched.",
      "echo.",
      "set \"REPLY=\"",
      ...
      ")",
      "",
      "rem Say which script is running, so the uninstaller leaves it alone: cmd reads a",
      "rem batch file line by line and errors out when the file disappears under it.",
      "set \"RP_UNINSTALL_SCRIPT=%~f0\"",
      "\"%EXE%\" --uninstall --purge",
      "echo.",
      "pause",
      "rem Last line, and the one thing a batch file is allowed to do to itself.",
      "del \"%~f0\" >nul 2>&1",
      "",
    ].join("\r\n")
  );
...
function writeUninstallBat(exeDir) {
  const target = path.join(exeDir, UNINSTALL_BAT_NAME);
  const text = uninstallBatText(path.basename(process.execPath));
  let current = "";
  try {
    current = fs.readFileSync(target, "latin1");
  } catch {}
  // Somebody else's uninstall.bat: leave it exactly as it is, forever.
  if (current && !UNINSTALL_BAT_MARKER.test(current)) return "";
  if (current === text) return "";
  try {
    fs.writeFileSync(target, text);
    return target;
  } catch (err) {
    console.log(`[sea] could not create ${UNINSTALL_BAT_NAME}: ${err.message}`);
    return "";
  }
}
...
function uninstall(exeDir, keepBackups, purge) {
  ...
  try {
    if (UNINSTALL_BAT_MARKER.test(fs.readFileSync(batPath, "latin1"))) {
      const running = (process.env.RP_UNINSTALL_SCRIPT || "").trim();
      scriptRemovesItself =
        Boolean(running) &&
        path.resolve(running).toLowerCase() === path.resolve(batPath).toLowerCase();
  ...
  if (purge) {
    ...
    console.log(
      `[uninstall] kept ${process.execPath}, run uninstall.bat or add --purge to remove it`
    );
  }
}
